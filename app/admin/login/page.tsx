...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post<{ message: string; token: string }>('/api/admin/login', formData);
      if (data.token) {
    ...
    <div className="grid place-items-center min-h-screen w-full">
      <div className="w-full max-w-sm px-6">
        <div className="text-center mb-8">
          <div className="inline-block mb-4 px-3 py-1 bg-amber-100 text-amber-800 text-xs font-semibold rounded-full">
            Admin Login
          </div>
          <h1 className="text-5xl font-bold text-primary mb-2">Admin Log In</h1>
        </div>
